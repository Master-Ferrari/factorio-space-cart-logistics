...
for _, class in ipairs(RM.CLASSES) do
  rail_protos[#rail_protos + 1] = {
    ...
    -- Ноль движок не принимает («crafting_speed must be positive»), а строку в
    -- тултипе рельса не убрать — прототипного флага для неё нет. Значение ни на что
    -- не влияет (рецептов нет, машина погашена), поэтому просто держим его мелким.
    -- Задавать crafting_speed_quality_multiplier ради гашения роста по качеству
    -- ПРОБОВАЛИ и откатили: явный словарь заставляет движок рисовать в тултипе
    -- целую таблицу «скорость по качествам» — лишний шум вместо одной строки.
    crafting_speed = 0.1,
