...
-- false (без явных условий захвата док кареток не берёт).
...
-- относительно КУРСА каретки: справа от головы > слева > сзади > перед
-- (лобовой, въезжает прямо в док, берётся последним); учитываются только
-- доки, ДЛЯ КОТОРЫХ каретка валидна (разные
...
-- Сможет ли каретка РЕАЛЬНО уехать с тайла tkey курсом exit — ровно условие
-- пересечения границы тайла в next_head (convoys): впереди есть рельс-узел И у
-- него при входе со стороны OPP(exit) есть выход (pick_exit ~= nil). Нет →
-- посадка на рельс заведомо провальна: каретка сядет и тут же упрётся в тупик,
-- гард last_released вернёт её через пару тиков («побег в никуда» — мигание на
-- рельсе в конце каждого круга карусели). try_drop этим не роняет каретку в
-- тупик (крутит карусель), а dock_remove — выбирает сторону выезда при сносе.
local function can_depart(tkey, exit)
  local node = storage.rails[G.neighbor_tile(tkey, exit)]
  return node ~= nil and R.pick_exit(node, G.OPP[exit]) ~= nil
end
...
function Docks.dock_remove(entity)
  ...
  if d.held then
    ...
    if cart then
      ...
      -- Снос дока с кареткой в руке: пробуем посадить её на целевой рельс. Здесь
      -- задача чисто ФИЗИЧЕСКАЯ (спасти каретку, если ей есть куда ехать), а не
      -- семантика захвата — поэтому курс по ТОПОЛОГИИ, не по геометрии дока:
      -- вперёд, если можно, иначе разворот, если открыто назад (can_depart —
      -- то же условие пересечения границы, что у движения). Некуда совсем /
      -- окно 32 клеток занято → не сядет → взрывается, как при блэкауте
      -- (`C.blackout_tile` по фактической мировой позиции; для каретки без
      -- состава/следа он сам падает на неё — cart_on_tile).
      local heading = d.heading or d.dir
      local exit = heading
      if not can_depart(d.tkey, heading) and can_depart(d.tkey, G.OPP[heading]) then
        exit = G.OPP[heading]
      end
...
-- Валидность подъезжающей каретки для захвата. Нет строк → false: без явных
-- условий захвата док кареток не берёт.
function Docks.grab_valid(ctx, d, un, cart)
  local conds = d.grab_conds
  if not (conds and #conds > 0) then return false end
...
-- едет): справа от хода > слева > сзади > перед (лобовой — каретка едет прямо
-- в него — берётся ПОСЛЕДНИМ: пусть боковые/задние доки разбирают проезжающие
-- каретки, а лобовой ловит только то, что иначе некому взять).
...
local function dock_rank(d, heading, served, forced)
  ...
  local rank = (side == G.CW[heading]) and 4
    or (side == G.CCW[heading]) and 3
    or (side ~= heading) and 2 or 1  -- сзади = 2, лоб (side == heading) = 1
...
-- Есть ли на тайле СВОБОДНЫЙ сосед (кроме from), который эту каретку в принципе
-- ВЗЯЛ БЫ (grab_valid / force_grab) — пусть даже уже обслуживал в этом круге.
-- Только тогда сброс круга ради продолжения карусели осмыслен. Наличие соседа
-- само по себе (idle-док с вечно-ложным условием) не считается: хендоффить
-- некому — держать каретку впустую незачем, пусть падает на рельс как обычно.
-- Отличие от pick_handoff: тот исключает уже обслуживших (ищет, КОМУ отдать
-- прямо сейчас); этот игнорирует эстафету (есть ли смысл открывать новый круг).
local function any_grabber(ctx, from, un, cart, doclist)
  if not doclist then return false end
  for _, d2 in ipairs(doclist) do
    if d2 ~= from and (un == d2.force_grab or Docks.grab_valid(ctx, d2, un, cart)) then
      return true
    end
  end
  return false
...
-- Визуальный разворот каретки на 180° (спрайт смотрит в сторону выезда). Один
-- раз за круг держания — `cart.dock_reversed` (флаг снимается при фактическом
-- отъезде на рельс), иначе каждый хоп карусели дёргал бы флип туда-сюда.
local function flip_cart(cart)
  if cart and not cart.dock_reversed and cart.entity and cart.entity.valid then
...
-- авто-триггер условий в step()). Заодно — заблаговременный (с первого кадра
-- выноса) визуальный разворот каретки, НО только у ЛОБОВОГО дока — того, в
-- который каретка едет прямо (`heading == OPP(dir)`, тот самый тупик). Боковые
-- и задний доки при передаче по кругу спрайт НЕ трогают: каретку они лишь
-- перекидывают дальше, а реальную посадку с разворотом сделает лобовой (в
-- очереди он последний — см. dock_rank). try_drop страхует тем же per-dock
-- условием (флип по факту exit ~= heading), но там оно тождественно этому →
-- по сути no-op. Раньше разворот и тут, и в try_drop шёл по ТАЙЛОВОМУ
-- «есть ли лобовой сосед» и срабатывал у ЛЮБОГО дока тайла — из-за этого
-- сквозной док разворачивал каретку лишь оттого, что рядом стоит лобовой
-- (баг из плейтеста): семантика разворота — свойство КОНКРЕТНОГО дока, не тайла.
-- `d.heading` НЕ трогаем: это «откуда каретка ехала», основа ранга хендоффа/
-- пасса 2b (перед/право/лево) — развернуть исход, не меняя его, иначе право и
-- лево у ранга поехали бы для будущих соседей.
local function start_release(d, cart)
  local heading = d.heading or d.dir
  if heading == G.OPP[d.dir] then flip_cart(cart) end  -- лобовой (тупик) — заранее
  d.state, d.arm = "lower", 0
  Docks.chest_unload(d)
end

-- 7.5/7.6: опустить пойманную каретку на целевой тайл. Курс exit — по
-- СОБСТВЕННОЙ геометрии дока: лобовой (каретка едет прямо в него) разворачивает,
-- сквозной/задний везёт тем же heading. Наличие соседних доков курс НЕ меняет.
-- Спрайт при инверсии доводит flip_cart (у лобового уже флипнут в start_release
-- → no-op). Если по exit тупик (`can_depart` = false), но на тайле есть сосед,
-- готовый ВЗЯТЬ каретку (any_grabber) — не роняем, крутим карусель. Иначе пока
-- прямой путь курса не существует / окно 32 клеток занято — держим кадр и ждём.
...
local function try_drop(d, held_cart, ctx, doclist, claim, handed)
  ...
  -- exit — по СОБСТВЕННОЙ геометрии дока, а НЕ по наличию соседей: лобовой
  -- (каретка едет прямо в него, heading == OPP(dir)) разворачивает — он терминал;
  -- сквозной/задний везёт каретку тем же курсом. Наличие тупикового дока рядом
  -- курс НЕ переопределяет (иначе сквозной док разворачивал бы каретку лишь
  -- оттого, что на тайле стоит лобовой сосед — баг из плейтеста; тайл виден всем
  -- докам одинаково, но семантика разворота — свойство КОНКРЕТНОГО дока).
  local exit = (heading == G.OPP[d.dir]) and G.OPP[heading] or heading
  if exit ~= heading then flip_cart(held_cart) end  -- лобовой: уже флипнут в start_release → no-op
  -- Бежать некуда (впереди по exit тупик — нет рельса/маршрут в стоп), а на тайле
  -- есть свободный сосед, который эту каретку ВЗЯЛ БЫ (any_grabber — важно: не
  -- «док просто есть», а «возьмёт»; сосед с вечно-ложным условием не в счёт): НЕ
  -- роняем ради заведомо провального побега (сядет → упрётся → гард вернёт через
  -- пару тиков = мигание на рельсе), а сбрасываем круг (прочие снова свежие) →
  -- следующий тик отдаст каретку соседу хендоффом, карусель крутится без выхода
  -- на рельс. Грабера-соседа нет (один эффективный док) — как обычно падаем на
  -- рельс: тупик → берёт-роняет по кругу (это правильное поведение).
  if not can_depart(d.tkey, exit) and any_grabber(ctx, d, d.held, held_cart, doclist) then
    mark_served(d, held_cart)
    return
  end
  local entry = G.OPP[exit]
  local node = storage.rails[d.tkey]
...
function Docks.on_tick()
  ...
  -- Приоритет дока — ОТНОСИТЕЛЬНО КУРСА каретки: справа от головы > слева >
  -- сзади > перед (лобовой берётся последним). Сторона на тайле уникальна (один
