...
-- (его dir == OPP(heading))? Это ФИЗИКА посадки: если впереди тупик-док, ехать
-- прямо некуда, кто бы каретку сейчас ни держал — поэтому проверка ТАЙЛОВАЯ, а
-- не «сам ли я лобовой». Нужна именно тайловой для случая, когда лобовой док
-- каретку НЕ берёт (условия false): тогда реальную посадку делает боковой, и
-- он обязан развернуть exit, хотя head-on не он. Определяет ТОЛЬКО exit-курс
-- (try_drop/dock_remove); ВИЗУАЛЬНЫЙ разворот спрайта заранее делает лишь
-- лобовой (start_release, per-dock heading == OPP(dir)) — боковые при передаче
-- по кругу спрайт не крутят, флип добивает try_drop по факту инверсии exit.
-- O(1): сосед по ходу однозначен по геометрии.
...
-- рельсе в конце каждого круга карусели). Используется try_drop, чтобы в этом
-- случае не ронять каретку, а крутить карусель дальше.
...
-- очереди он последний — см. dock_rank); если же лобовой каретку не берёт
-- (условия false) и садит боковой — разворот добьёт try_drop по факту инверсии
-- курса (там проверка остаётся ТАЙЛОВОЙ — физически ехать прямо всё равно
-- некуда). Раньше разворот тут был по тайловому heading_blocked и срабатывал у
-- ЛЮБОГО дока тайла — из-за этого боковой док разворачивал каретку ещё до того,
-- как её схватит лобовой (баг из плейтеста).
...
-- в чужую клешню (heading_blocked — ТАЙЛОВАЯ проверка: если впереди по курсу
-- стоит док, целящийся навстречу, ехать прямо некуда, кто бы каретку сейчас ни
-- держал) — инвертируем exit, чтобы уехала. Спрайт при инверсии доводит
-- flip_cart тут же (у лобового флип уже случился в start_release — no-op; у
-- НЕлобового, если сажает именно он, — здесь по факту). Пока целевой прямой
...
local function try_drop(d, held_cart, ctx, doclist, claim, handed)
  ...
  local exit = heading_blocked(d.tkey, heading) and G.OPP[heading] or heading
  -- фолбэк спрайта: реальную посадку с инверсией курса делает НЕлобовой док
  -- (лобовой не взял каретку — условия false), а он спрайт в start_release не
  -- разворачивал; курс тут тайловый (ехать прямо всё равно некуда) — приводим
  -- спрайт к нему. У лобового флип уже случился заранее (dock_reversed) → no-op.
  if exit ~= heading then flip_cart(held_cart) end
  -- Бежать некуда (впереди по exit тупик — нет рельса/маршрут в стоп), а на
  -- тайле есть свободный сосед: НЕ роняем каретку ради заведомо провального
  -- побега (сядет → упрётся → гард вернёт через пару тиков = «мигание» на рельсе
  -- в конце каждого круга), а сбрасываем круг (mark_served текущего = прочие
  -- доки снова свежие) — следующий тик отдаст каретку соседу хендоффом, карусель
  -- крутится дальше без выхода на рельс. Одиночный док (соседа в doclist нет) —
  -- как раньше: пробуем сесть (держать вечно в одиночку смысла нет).
  if not can_depart(d.tkey, exit) and doclist and #doclist > 0 then
